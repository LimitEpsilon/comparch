...
class HazardUnit extends Module {
  ...
  val ifid_bubble  = Wire(Bool())
  val idex_bubble  = Wire(Bool())
  val exmem_bubble = Wire(Bool())
  val ifid_flush   = Wire(Bool())

  // Load to use hazard.
  when (io.exmem_taken === true.B) { // branch taken
    pcwrite := 1.U
    ifid_bubble := false.B
    idex_bubble := true.B
    exmem_bubble := true.B
    ifid_flush := true.B
  } .elsewhen (io.idex_memread && ((io.idex_rd === io.rs1) || (io.idex_rd === io.rs2))) { // insn in EX: ld
    pcwrite := 2.U
    ifid_bubble := true.B
    idex_bubble := true.B
    exmem_bubble := false.B
    ifid_flush := false.B
  } .otherwise {
    pcwrite := 0.U
    ifid_bubble := false.B
  ...
  // branch flush
  io.pcwrite := pcwrite
  io.ifid_bubble := ifid_bubble
  io.idex_bubble := idex_bubble
  io.exmem_bubble := exmem_bubble
  io.ifid_flush := ifid_flush
}
